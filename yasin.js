...
async function loadYasinData() {
    try {
        const response = await fetch('yasin_data.json');
...
function displayVerse(verseData) {
    ...
    verseContainer.innerHTML = `
        <p class="text-4xl text-right font-arabic leading-loose mb-6 text-yellow-300">${arabicText}</p>
        <p class="text-xl mb-4 text-blue-300 font-semibold">${transliteration}</p>
        <p class="text-lg text-gray-100">${translation}</p>
...
function searchVerse(keyword) {
    for (let i = 0; i < yasinData.length; i++) {
        const verseText = yasinData[i].find(v => v.edition.language === 'id')?.text || '';
