...
def sanitize_filename(filename):
    return re.sub(r'[^\w\-_\. ]', '', filename).replace(' ', '_').lower()
...
if __name__ == "__main__":
    ...
    for index, (surah_name, (surah_number, total_verses)) in enumerate(surahs.items(), 1):
        surah_data = fetch_surah_verses(surah_number, total_verses)
        if surah_data:
            sanitized_name = sanitize_filename(surah_name)
            filename = os.path.join(output_dir, f"{index:02d}_{sanitized_name}_data.json")
